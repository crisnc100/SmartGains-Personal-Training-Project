...
const EditExerciseModal = ({ onClose, exercise, onUpdateExercise }) => {
  const user = useUser();
  const [exerciseData, setExerciseData] = useState({
    ...
    useEffect(() => {
        if (exercise) {
            ...
            if (exercise.gif_url) setGifOption('link');
            if (exercise.video_url) setVideoOption('link');
        }
    }, [exercise]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:5000/api/update_custom_exercise/${exercise.id}`, formData, {
                headers: {
            ...
            console.log("Edit Exercise response:", response.data);
            onUpdateExercise(response.data.updatedExercise); // Assuming your backend sends back the updated exercise
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-700 bg-opacity-50 z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm mx-auto">
                <h2 className="text-xl font-bold mb-4">{exercise ? 'Edit Exercise' : 'Add New Exercise'}</h2>
