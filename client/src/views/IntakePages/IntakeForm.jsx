...
const IntakeForm = () => {
    ...
    useEffect(() => {
        ...
        const savedFormId = localStorage.getItem('formId'); // Retrieve formId from local storage
        ...
        if (savedCurrentQuestions) {
            try {
                ...
                if (Array.isArray(parsedQuestions)) {
                    ...
                    initializeForm(questionsWithSource, savedAnswers ? JSON.parse(savedAnswers) : {});
                } else {
    ...
    const fetchDefaultQuestions = async () => {
        try {
            ...
            setQuestions(questionsWithSource);
            localStorage.setItem('currentQuestions', JSON.stringify(questionsWithSource)); // Save questions to local storage
    ...
    const handleInputChange = (event, questionId) => {
        ...
        localStorage.setItem('currentAnswers', JSON.stringify(updatedForm));
    };
    ...
    const handleCheckboxChange = (event, questionId) => {
        ...
        setIntakeForm((prevState) => {
            ...
            const updatedForm = { ...prevState, [questionId]: updatedArray };
            localStorage.setItem('currentAnswers', JSON.stringify(updatedForm));
            return updatedForm;
        });
    };
    ...
    useEffect(() => {
        const autoSave = async () => {
            if (!formId) {
                // Create a new form if formId is not set
                try {
                    const formResponse = await axios.post('http://localhost:5000/api/auto_save_intake_form', {
                        withCredentials: true,
                        client_id: clientId,
                        form_data: { form_type: 'initial consultation', status: 'draft' },
                        answers: []
                    });
                    if (formResponse.data.form_id) {
                        const newFormId = formResponse.data.form_id;
                        console.log(`Auto-save: New form_id created ${newFormId}`);
                        setFormId(newFormId);
            ...
            try {
                ...
                localStorage.setItem('currentAnswers', JSON.stringify(intakeForm)); // Save answers to local storage
                ...
                if (response.data.form_id) {
                    ...
                    localStorage.setItem('formId', response.data.form_id);
                }
            } catch (error) {
    ...
    const handleSubmit = async (e, skipAI = false) => {
        ...
        try {
            // Step 1: Save the intake form (if not already saved)
            const formResponse = formId
                ? { data: { id: formId } }
                : await axios.post('http://localhost:5000/api/add_intake_forms', {
                    withCredentials: true,
            ...
            // Step 4: Navigate based on the button clicked
            if (skipAI) {
                navigate('success-intake_options'); // Skipping AI insights
            } else {
                navigate('success-overview'); // Generate and show AI insights
            }
        } catch (error) {
    ...
    const handleCustomize = () => {
        ...
        localStorage.setItem('currentAnswers', JSON.stringify(intakeForm));
                ...
                <form onSubmit={(e) => handleSubmit(e, false)} className="w-full max-w-4xl">
                    ...
                    <div className="flex justify-between">
                        <button type="button" onClick={(e) => handleSubmit(e, true)} className="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm w-full p-2.5 text-center mt-6 mr-2" disabled={submitting}>
                            {submitting ? "Submitting..." : "Submit and Skip AI Insights"}
                        </button>
                        <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm w-full p-2.5 text-center mt-6 ml-2" disabled={submitting}>
                            {submitting ? "Submitting..." : "Submit and Get AI Insights"}
                        </button>
                    </div>
                </form>
            </div>
