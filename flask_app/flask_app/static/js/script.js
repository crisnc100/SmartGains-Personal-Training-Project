...
function confirmDemoDelete(clientId, demoId, csrfToken) {
    if (confirm('Are you sure you want to delete this plan?')) {
        var form = document.createElement('form');
        document.body.appendChild(form);
        form.method = 'post';
        form.action = `/delete_demo_plan/${clientId}/${demoId}`;
...
function confirmGeneratedDelete(clientId, generatedId, csrfToken) {
    if (confirm('Are you sure you want to delete this plan?')) {
        var form = document.createElement('form');
        document.body.appendChild(form);
        form.method = 'post';
        form.action = `/delete_client_plan/${clientId}/${generatedId}`;
        

        // Create a hidden field for the CSRF token
        var token = document.createElement('input');
        token.type = 'hidden';
        token.name = 'csrf_token';
        token.value = csrfToken;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    deleteButton.addEventListener('click', confirmClientDelete);
});

function confirmClientDelete() {
    var button = this;  // 'this' refers to the button clicked
    ...
    if (!clientId) {
        alert("Client ID is missing, cannot proceed with deletion.");
        return;
    }

    if (confirm(`Are you sure you want to delete ${clientName} as a client?`)) {
        var form = document.createElement('form');
        document.body.appendChild(form);
        form.method = 'post';
        form.action = `/delete_client/${clientId}`;
