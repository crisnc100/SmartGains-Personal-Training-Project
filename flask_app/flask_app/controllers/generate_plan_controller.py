...
from flask_app.models.initial_prompts_model import InitialPrompts
...
def get_recent_quick_plan(client_id):
    ...
    return jsonify({
        ...
        "demo_plan": {
            ...
            "demo_plan_date": demo_plan.created_at.strftime('%Y-%m-%d'),  
...
def generate_custom_plan(client_id):
    ...
    data = request.get_json()  
    trainers_prompt = data.get('description')
    if not trainers_prompt:
        return jsonify({"success": False, "message": "Prompt data is missing."})
    print("Received trainers prompt:", trainers_prompt)  

    final_prompt = trainers_prompt  

# Instructions for how the AI should structure the workout plan
    additional_instructions = """\n\nPlease format the workout plan with clear headers for each part of the 
    workout, including specific exercises, sets, reps, and rest periods. Detail the intensity of weights where 
    applicable and provide alternatives for each exercise if the specified equipment is not available. 
    Ensure the language is clear and concise, suitable for both beginners and experienced individuals, 
    focusing on safety and effective progression."""

    final_prompt += additional_instructions
...
def get_recent_custom_plan(client_id):
    ...
    return jsonify({
        ...
        "generated_plan": {
            ...
            "generated_plan_date": generated_plan.created_at.strftime('%Y-%m-%d'),  
