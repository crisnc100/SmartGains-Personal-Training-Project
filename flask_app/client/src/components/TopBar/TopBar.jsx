...
const TopBar = ({ sidebarToggle, setSidebarToggle, sidebarWidth }) => {
  ...
  return (
    ...
    <div style={{ 
        position: 'absolute', 
        left: sidebarToggle ? '10px' : `calc(${sidebarWidth} + 10px)`,  // Adjusts left margin based on sidebar
        display: 'flex', 
        alignItems: 'center' 
    }}>
...
<span className='font-semibold' 
      style={{ 
          fontSize: '25px',
          fontWeight: 'bolder',
          color: '#a7e2ff', // Set the text color to blue
          textShadow: `3px 3px 4px rgba(0, 0, 0, 0.8),
                       -1px -1px 4px rgba(0, 0, 0, 0.8),
                       1px -1px 4px rgba(0, 0, 0, 0.8),
                       -1px 1px 4px rgba(0, 0, 0, 0.8),
                       1px 1px 4px rgba(0, 0, 0, 0.8)` // Adding the text-shadow
      }}>
  FitForge
</span>
    </div>
      <div style={{ flex: 1.0 }}></div> {/* This div remains as a spacer */}
