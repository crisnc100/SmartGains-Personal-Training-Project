...
const IntakeForm = () => {
                ...
                <div className="flex justify-between">
                    {Array.from({ length: 10 }, (_, i) => i + 1).map(value => (
                        <label key={value} className="inline-flex flex-col items-center" style={{ color: 'black' }}>
                            <span className="text-sm">{value}</span> {/* Display the number */}
                            <input
                                type="radio"
                                name={key}
                                value={value}
    ...
    const validateForm = () => {
        ...
        Object.keys(intakeForm).forEach(key => {
            // Ensure the value is a string and check if it's empty
    ...
    const submitHandler = async (e) => {
        ...
        try {
            ...
            navigate('medical-history'); // Ensure the path is correctly set
        } catch (err) {
            ...
            if (err.response) {
                ...
                setErrors(prevErrors => ({
                    ...prevErrors,
                    ...err.response.data.errors, // Make sure the backend is structured to send back an `errors` object
                }));
            } else {
                // Handle cases where the error response is not received
                setErrors(prevErrors => ({
                    ...prevErrors,
