...
const DemoPrompt = () => {
  ...
  const [selectedPrompt, setSelectedPrompt] = useState(null);  // Ensure it’s initialized properly
  ...
  const validateForm = () => {
    ...
    let valid = true;
    if (!selectedPrompt) {
      // Set an error if no prompt is selected
  ...
  const submitHandler = async (e) => {
    ...
    const data = {
        promptContent: selectedPrompt.content,  // This should now be an HTML string
    ...
    try {
        ...
        navigate(`success`);  // Adjust this to match the actual response structure
    } catch (err) {
