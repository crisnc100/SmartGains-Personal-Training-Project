...
class WorkoutPrompts:
    ...
    @classmethod
    def save(cls, data):
        query = """INSERT INTO workout_prompts (level, prompt_template, created_at, updated_at, trainer_id, client_id) 
    ...
    def get_by_trainer_id(cls, trainer_id):
        ...
        if result:
            return [cls(row) for row in result]  # Return a list of WorkoutPrompts objects
        else:
            return None
    ...
    def delete(cls, prompt_id):
        query = "DELETE FROM workout_prompts WHERE id = %(id)s;"
        data = {"id": prompt_id}
        return connectToMySQL('fitness_consultation_schema').query_db(query, data)